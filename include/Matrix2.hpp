...
public:
    ...
    constexpr Vector<Type, 2> getFirstCol() const noexcept {
        ...
        if constexpr (same_type<Type, float>) {
            _mm256_store_ps((float*)&firstCol,
            _mm256_permute_ps(*(__m256*)this, 0b1000));
        } else {
            _mm256_store_si256((__m256i*)&firstCol,
            _mm256_shuffle_epi32(*(__m256i*)this, 0b1000));
        }
        return firstCol;
    }
    ...
    constexpr Vector<Type, 2> getSecondCol() const noexcept {
        ...
        if constexpr (same_type<Type, float>) {
            _mm256_store_ps((float*)&secondCol,
            _mm256_permute_ps(*(__m256*)this, 0b1101));
        } else {
            _mm256_store_si256((__m256i*)&secondCol,
            _mm256_shuffle_epi32(*(__m256i*)this, 0b1101));
        }
        return secondCol;
    }
    ...
    static constexpr Mat2 identity() noexcept {
        ...
        Mat2 identityMat;
        if constexpr (same_type<Type, float>) {
            _mm256_store_ps((float*)&identityMat,
            _mm256_set_ps(0,0,0,0,1,0,0,1));
        } else {
            _mm256_store_si256((__m256i*)&identityMat,
            _mm256_set_epi32(0,0,0,0,1,0,0,1));
        }
        return identityMat;
    }
    ...
    constexpr void transposed() noexcept {
        ...
        if constexpr (same_type<Type, float>) {
            ...
            _mm256_permute_ps(*(__m256*)this, 0b11011000));
        } else {
            _mm256_store_si256((__m256i*)this,
            _mm256_shuffle_epi32(*(__m256i*)this, 0b11011000));
        }
    }
    ...
    */
    ...
    constexpr Vector<Type, 2> operator*(const Vector<Type, 2> vec) const noexcept {
        ...
        if constexpr (same_type<Type, float>) {
            const auto temp = _mm256_mul_ps(*(__m256*)this,
                              _mm256_permute_ps(*(__m256*)&vec, 0b01000100));
            _mm256_store_ps((float*)&result, _mm256_hadd_ps(temp, temp));
        } else {
            const auto temp = _mm256_mullo_epi32(*(__m256i*)this,
                              _mm256_shuffle_epi32(*(__m256i*)&vec, 0b01000100));
            _mm256_store_si256((__m256i*)&result, _mm256_hadd_epi32(temp, temp));
        }
        return result;
    }
