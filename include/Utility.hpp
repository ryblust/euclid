...
EuclidForceinline constexpr Vector<T, S> clamp(const Vector<T, S> vec, const Vector<T, S> min, const Vector<T, S> max) noexcept {
    if (__builtin_is_constant_evaluated()) {
        ...
        if constexpr (same_type<T, float>) {
            _mm256_store_ps((float*)&clampVec, _mm256_max_ps(_mm256_min_ps(*(__m256*)&vec, *(__m256*)&max), *(__m256*)&min));
        } else {
            _mm256_store_si256((__m256i*)&clampVec, _mm256_max_epi32(_mm256_min_epi32(*(__m256i*)&vec, *(__m256i*)&max), *(__m256i*)&min));
        }
        return clampVec;
    }
}
...
EuclidForceinline constexpr Vector<T, S> clamp(const Vector<T, S> vec, const T min, const T max) noexcept {
    if (__builtin_is_constant_evaluated()) {
        ...
        Vector<T, S> clampVec;
        if constexpr (same_type<T, float>) {
            _mm256_store_ps((float*)&clampVec, _mm256_max_ps(_mm256_min_ps(*(__m256*)&vec, _mm256_set1_ps(max)), _mm256_set1_ps(min)));
        } else {
            _mm256_store_si256((__m256i*)&clampVec, _mm256_max_epi32(_mm256_min_epi32(*(__m256i*)&vec, _mm256_set1_epi32(max)), _mm256_set1_epi32(min)));
        }
        return clampVec;
    }
}
...
EuclidForceinline constexpr Vector<float, S> lerp(const Vector<float, S> a, const Vector<float, S> b, const Vector<float, S> t) noexcept {
    if (__builtin_is_constant_evaluated()) {
        ...
        Vector<float, S> lerpVec;
        _mm256_store_ps((float*)&lerpVec, _mm256_fmadd_ps(*(__m256*)&t, _mm256_sub_ps(*(__m256*)&b, *(__m256*)&a), *(__m256*)&a));
        return lerpVec;
    }
}
...
EuclidForceinline constexpr Vector<float, S> lerp(const Vector<float, S> a, const Vector<float, S> b, const float t) noexcept {
    if (__builtin_is_constant_evaluated()) {
        ...
        Vector<float, S> lerpVec;
        _mm256_store_ps((float*)&lerpVec, _mm256_fmadd_ps(_mm256_set1_ps(t), _mm256_sub_ps(*(__m256*)&b, *(__m256*)&a), *(__m256*)&a));
        return lerpVec;
    }
}
...
EuclidForceinline constexpr Vector<T, S> saturate(const Vector<T, S> vec) noexcept {
    if (__builtin_is_constant_evaluated()) {
        ...
        Vector<T, S> saturateVec;
        if constexpr (same_type<T, float>) {
            _mm256_store_ps((float*)&saturateVec, _mm256_max_ps(_mm256_min_ps(*(__m256*)&vec, _mm256_set1_ps(1.f)),_mm256_setzero_ps()));
        } else {
            _mm256_store_si256((__m256i*)&saturateVec, _mm256_max_epi32(_mm256_min_epi32(*(__m256i*)&vec, _mm256_set1_epi32(1.f)), _mm256_setzero_si256()));
        }
        return saturateVec;
    }
}
