...
struct PermuteHelper {
    ...
    static EUCLID_CONSTEXPR Vec4 EUCLID_CALL default_permute(const Vec4 v) noexcept {
        #ifndef __clang__
...
static EUCLID_CONSTEXPR Vec4 EUCLID_CALL permute(const Vec4 v) noexcept {
    #ifndef __clang__
    if (__builtin_is_constant_evaluated()) {
    ...
    template<std::size_t x, std::size_t y, std::size_t z, std::size_t w>
    static constexpr bool is_permute_match =
...
struct ShuffleHelper {
    ...
    static Vec4 EUCLID_CALL shuffle_helper(const Vec4 a, const Vec4 b) noexcept {
...
EUCLID_QUALIFIER Vec4 EUCLID_CALL permute(const Vec4 v) noexcept {
