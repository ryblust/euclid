...
EUCLID_QUALIFIER Mat2 EUCLID_CALL operator*(Mat2 a, Mat2 b) noexcept {
    ...
    const auto xzxz = _mm_shuffle_ps(b.mat, b.mat, _MM_SHUFFLE(2, 0, 2, 0));
    const auto ywyw = _mm_shuffle_ps(b.mat, b.mat, _MM_SHUFFLE(3, 1, 3, 1));
    const auto mul1 = _mm_mul_ps(a.mat, xzxz);
    const auto mul2 = _mm_mul_ps(a.mat, ywyw);
    const auto hadd = _mm_hadd_ps(mul1, mul2);
